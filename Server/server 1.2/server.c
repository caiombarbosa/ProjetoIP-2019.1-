...
#define lTela 420 //Largura da tela
...
int main() {
  ...
  fonte = al_load_font("pressStart.tff", 48, 0);

  // Preenchemos a tela com a cor preta
  al_clear_to_color(al_map_rgb(0, 0, 0)); 
  // Texto centralizado
  al_draw_text(fonte, al_map_rgb(255, 255, 255), lTela / 2, 90, ALLEGRO_ALIGN_CENTRE, "Esperando\njogadores");
  ...
  while (1) {
    ...
    if (id == 0) al_clear_to_color(al_map_rgb(0, 0, 0)); //limpa a mensagem inicial
    ...
    if (id != NO_CONNECTION) {
      ...
      al_draw_text(fonte, al_map_rgb(255, 255, 255), lTela / 2, 70 + id*65, ALLEGRO_ALIGN_CENTRE, jogador[id].login);
    }
    ...
    if (msg_ret.status == MESSAGE_OK) {
      ...
      for(i=0; i <= maxPessoasNoChat; i++){
        if(jogador[i].id == msg_ret.client_id){
          ...
          al_draw_text(fonte, al_map_rgb(255, 255, 255), lTela / 2, 70 + aux*65, ALLEGRO_ALIGN_CENTRE, jogador[i].login);
          aux++;
        }
      }
