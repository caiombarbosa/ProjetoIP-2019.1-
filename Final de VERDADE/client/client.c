...
void desenhaSpritePersonagemJogador2(char magiaPos, int pos_x_sprite, int pos_y_sprite){
    al_set_target_bitmap(fundo);
    al_set_target_backbuffer(janela);
    	if(magiaPos == 'C'){
            if(andandoCima[0] == 1){
                al_draw_bitmap_region(folha_sprite_Bianca,
                    largura_sprite*0,  (altura_sprite)*3, // posicao para tirar da folha dos sprites
                    largura_sprite,(altura_sprite)*3, // tamanho para tirar
                    pos_x_sprite,pos_y_sprite,0); //  onde colocar a imagem, flags
            }
...
bool inicializa(){
    ...
    over = al_load_bitmap("gameover.png");
    if(!over){
        error_msg("Falha ao incializar a tela de gamover!");
        return 0;
    }
    ...
    loading = al_load_bitmap("fundoPreto.jpg");
...
enum conn_ret_t tryConnect() {
  ...
  while (!sair && !concluido){
    ...
    if (!concluido){
        al_draw_text(fonte1, al_map_rgb(255, 255, 255), LARGURA_TELA / 2,(ALTURA_TELA / 2) / 2, ALLEGRO_ALIGN_CENTRE, "Digite o server IP: ");
    }
...
void runChat() {
  ...
  while (!sair && !sairChat) {
      ...
      while(!al_is_event_queue_empty(fila_eventos1)){
        ...
        if (!concluido){
            ...
            if (evento.type == ALLEGRO_EVENT_KEY_DOWN && (evento.keyboard.keycode == ALLEGRO_KEY_LCTRL || evento.keyboard.keycode == ALLEGRO_KEY_RCTRL)){
                char msgSaiServer[10] = "65561";
                sendMsgToServer(msgSaiServer, (int)strlen(msgSaiServer) + 1);
                sairChat = true;
            }
        }
    ...
    strcpy(msgServer, "");
      ...
      al_draw_text(fonte2, al_map_rgb(255, 255, 255), 5, ALTURA_TELA-tamFonte1, ALLEGRO_ALIGN_LEFT, msg);//Printa a mensagem na tela
...
int main(void){
    ...
    fonte = al_load_font("pressStart.ttf", 72, 0); // fonte do loading escrita, ela bugava quando eu colocava fora da main, entao ficou aqui
    if (fonte == NULL){
...
/////////////////////////////////////////   VARIAVEIS DA MAIN PARA O JOGO   ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    ///Camera 
        ...
        recvMsgFromServer(&connected, WAIT_FOR_IT);
        printf("entrei no jogo\n");
        ret = recvMsgFromServer(&id, WAIT_FOR_IT);
        if (ret == SERVER_DISCONNECTED){
            sair = true;
        }
        //printf("id :%d\n", id);

        ret = recvMsgFromServer(players, WAIT_FOR_IT);
        if (ret == SERVER_DISCONNECTED){
            sair = true;
        }
        personagem = players[i].character;
        //Posição inicial do jogador e da câmera;
        int pos_x_sprite = players[id].posxInicial, pos_y_sprite = players[id].posyInicial;
        
        int pos_x_camera = players[id].posxInicial, pos_y_camera = players[id].posyInicial;

        printf("pos incial x :%d \n", players[id].posxInicial);
        printf("pos incial y :%d \n", players[id].posyInicial);

        while(!sair){
            
            //printf("ESSE EH O ID: %d", id);
    ...
    //----------------------------------------------------//  SECÃO DE MOVIMENTACAO E MAGIAS //----------------------------------------------//
        ...
        ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                ...
                if ( pos_x_sprite >  1150){
                //inverte o sentido da velocidade X, para andar no outro sentido
                vel_x_camera = -vel_x_camera;
                vel_x_sprite = -vel_x_sprite;
                }
                // se o sprite chegar perto dos limites horizontais
                if( pos_y_sprite + altura_sprite > altura_mapa - 100 || pos_y_sprite < 20 ) {
...
//////////////////////////////////////////////////////////////////////////////////////////////////////* -- ATUALIZA TELA -- *///////////////////////////////////////////////////////////////////////////////
            if(desenha && al_is_event_queue_empty(fila_eventos)) {
                ...
                // atualiza a strct do jogador no server
                players[id].posX = pos_x_sprite;
                players[id].posY = pos_y_sprite;
                players[id].velX = vel_x_sprite;
                players[id].velY = vel_y_sprite;
                players[id].virado = magiaPos;
                players[id].magia = tecla;

                ret = sendMsgToServer(&players[id], sizeof(Jogador));
                if(ret == SERVER_DISCONNECTED){
                    sair = true;
                }
                ...
                //printf("pos x : %d, pos y: %d\n", players[id].posX, players[id].posY);
    ...
    //--------------------------------------------//////////// *-- MAGIA DE GELO (X) ////////////////--------------------------------------------/////
        ...
        /////////////////////// *-- DESENHANDO CAIO BLACK --* ///////////////////////////////////////////////////////////////////////////////////////////////////// 
                ...
                for(int j = 0; j < connected; j++){
                    //printf("jogado %d : pos x %d pos y %d\n", j, players[j].posX, players[j].posY);
                        if(j != id){
                        int posxJogador, posyJogador;                    
                        posxJogador = players[j].posX;
                        posyJogador = players[j].posY;
                        //printf("jogador %d, pos x : %d, pos y : %d", j, posxJogador, posyJogador);
                        // printando o player 2
                        desenhaSpritePersonagemJogador2(players[j].virado, players[j].posX, players[j].posY);
                        printf("virado aqui :%c\n", players[1].virado);
                        //desenhaSpritePersonagem(magiaPos, pos_x_sprite, pos_y_sprite);
                        if(players[j].magia == 5){
                            players[j].magia = 0;
                            al_set_target_bitmap(fundo);
                            al_set_target_backbuffer(janela);
                            if(players[j].virado == 'B'){
                                    printf("printando magia aqui!\n");
                                    int cooldown = framesTotal - frameReferencia;
                                        al_draw_bitmap_region(magia_Fire,
                                        regiao_x_magia_Z,regiao_y_magia_Z+(128*(cooldown-1)),
                                        largura_magia,altura_magia,
                                        players[j].posX,players[j].posY + 400, 0);
                                        al_draw_bitmap_region(magia_Fire,
                                        regiao_x_magia_Z,regiao_y_magia_Z+(128*(cooldown-1)),
                                        largura_magia,altura_magia,
                                        players[j].posX,players[j].posY + 350, 0);
                                        al_draw_bitmap_region(magia_Fire,
                                        regiao_x_magia_Z,regiao_y_magia_Z+(128*(cooldown-1)),
                                        largura_magia,altura_magia,
                                        players[j].posX,players[j].posY + 300, 0);
                                        al_draw_bitmap_region(magia_Fire,
                                        regiao_x_magia_Z,regiao_y_magia_Z+(128*(cooldown-1)),
                                        largura_magia,altura_magia,
                                        players[j].posX,players[j].posY + 250, 0);
                                        al_draw_bitmap_region(magia_Fire,
                                        regiao_x_magia_Z,regiao_y_magia_Z+(128*(cooldown-1)),
                                        largura_magia,altura_magia,
                                        players[j].posX,players[j].posY + 200, 0);
                                        al_draw_bitmap_region(magia_Fire,
                                        regiao_x_magia_Z,regiao_y_magia_Z+(128*(cooldown-1)),
                                        largura_magia,altura_magia,
                                        players[j].posX,players[j].posY + 150, 0);
                                        al_draw_bitmap_region(magia_Fire,
                                        regiao_x_magia_Z,regiao_y_magia_Z+(128*(cooldown-1)),
                                        largura_magia,altura_magia,
                                        players[j].posX,players[j].posY + 100, 0);
                            }
                            // PARA CIMA
                            if(players[j].virado == 'C'){
                                    printf("printando magia aqui!\n");
                                    int cooldown = framesTotal - frameReferencia;
                                    printf("%d", cooldown);
                                        al_draw_bitmap_region(magia_Fire,
                            ...
                            if(players[j].virado == 'D'){
                                    ...
                                    printf("%d", cooldown);
                                        ...
                                        regiao_x_magia_Z,regiao_y_magia_Z+(128*(cooldown-1)),
                                        largura_magia,altura_magia,
                                        players[j].posX + 400,players[j].posY, 0);
                                        al_draw_bitmap_region(magia_Fire,
                                        regiao_x_magia_Z,regiao_y_magia_Z+(128*(cooldown-1)),
                                        largura_magia,altura_magia,
                                        players[j].posX + 350,players[j].posY, 0);
                                        al_draw_bitmap_region(magia_Fire,
                                        regiao_x_magia_Z,regiao_y_magia_Z+(128*(cooldown-1)),
                                        largura_magia,altura_magia,
                                        players[j].posX + 300,players[j].posY, 0);
                                        al_draw_bitmap_region(magia_Fire,
                                        regiao_x_magia_Z,regiao_y_magia_Z+(128*(cooldown-1)),
                                        largura_magia,altura_magia,
                                        players[j].posX + 250,players[j].posY, 0);
                                        al_draw_bitmap_region(magia_Fire,
                                        regiao_x_magia_Z,regiao_y_magia_Z+(128*(cooldown-1)),
                                        largura_magia,altura_magia,
                                        players[j].posX + 200,players[j].posY, 0);
                                        al_draw_bitmap_region(magia_Fire,
                                        regiao_x_magia_Z,regiao_y_magia_Z+(128*(cooldown-1)),
                                        largura_magia,altura_magia,
                                        players[j].posX + 150,players[j].posY, 0);
                                        al_draw_bitmap_region(magia_Fire,
                                        regiao_x_magia_Z,regiao_y_magia_Z+(128*(cooldown-1)),
                                        largura_magia,altura_magia,
                                        players[j].posX + 100,players[j].posY, 0);
                            }
                            // PARA ESQUERDA
                            if(players[j].virado == 'E'){
                                    printf("printando magia aqui!\n");
                                    int cooldown = framesTotal - frameReferencia;
                                    printf("%d", cooldown);
                                        al_draw_bitmap_region(magia_Fire,
            ...
            printf("magia do personagem : %d\n", players[1].magia);
            if(players[id].vida == 0){
                printf("Estou saindo deste plano!\n");
