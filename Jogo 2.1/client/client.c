...
void carregar_arquivos(){
    ...
    */
    ...
    start_0 = al_load_bitmap("start_0.jpg");
    if(start_0 == NULL){
    ...
    start_1 = al_load_bitmap("start_1.jpg");
    if(start_1 == NULL){
    ...
    start_2 = al_load_bitmap("start_2.jpg");
    if(start_2 == NULL){
        error_msg("Falha na start1");
        al_destroy_display(janela);
        sair = true;
    ...
    start_3 = al_load_bitmap("start_3.jpg");
    start_4 = al_load_bitmap("start_4.jpg");
...
void carregaAudios(){
    somMenu = al_load_audio_stream("openingSom.mp3",4,1024);
    if(somMenu==NULL){
       error_msg("Falha ao iniciar somMenu");
    }

    opening = al_load_audio_stream("musicaMenu.ogg",4,1024);
    if(opening==NULL){
        error_msg("Falha ao iniciar opening");
    }

    /*
    you_lose_sample = al_load_sample("");
    you_win_sample = al_load_sample("");
    cheater_sample = al_load_sample("");
    timesup_sample = al_load_sample("");
    ...
    if(cheater_sample==NULL){
        error_msg("Falha ao iniciar cheater");
        return false;
    }
...
int main(void){
    ...
    //variaveis para manipular ele na hora de printar quantos sprites tem em cada linha da folha, e a atualmente mostrada
    int coluna_atual=0;
    ...
    loading = al_load_bitmap("start_1.jpg");
    if(loading == NULL){
    ...
    char codStartServer[] = "65561";
    ret = sendMsgToServer(codStartServer, strlen(codStartServer) + 1);//Mensagem iniciar jogo para o servidor
        if (ret == SERVER_DISCONNECTED) {
            return -1;}
    ...
    //=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- EVENTOS DO TECLADO =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-//
        ...
        if(desenha && al_is_event_queue_empty(fila_eventos)) {
            ...
            //=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- DESENHANDO AS MAGIAS QUE FORAM USADAS =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-//
            ...
            ret = sendMsgToServer(&PlayerOne, sizeof(Jogador)); //ret recebe a quantidade de bytes enviada
            if (ret == SERVER_DISCONNECTED) {
                return -1;}

            for(i=0; i<3; i++){
                ret = recvMsgFromServer(&outros_players[i], DONT_WAIT);//o vetor outros_players vai armazenar as informacoes dos outros jogadores
                if (ret == SERVER_DISCONNECTED) {
                    return -1;}
            }
            al_flip_display();
