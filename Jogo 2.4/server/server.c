...
int main() {
  ...
  while (!sair){
    ...
    if(jogar){
      ...
      pessoa[0].character = 1;

      pessoa[1].posxInicial = 500;
      pessoa[1].posyInicial = 500;
      pessoa[1].character = 1;

      pessoa[2].posxInicial = 890;
      pessoa[2].posyInicial = 350;
      pessoa[2].character = 1;

      pessoa[3].posxInicial = 890;
      pessoa[3].posyInicial = 600;
      pessoa[3].character = 1;
    }
    ...
    while (jogar && !sair){
      ...
      for(i=0; i<connected; i++){
        struct msg_ret_t msg_ret = recvMsgFromClient(&pessoa[i], i, DONT_WAIT);
