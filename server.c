        ...
        if(msg_ret.status != DISCONNECT_MSG){
          for(j = 0; j < connected; j++){
            ...
            if(j != msg_ret.client_id){
              if(pessoa[msg_ret.client_id].magia == 6){
                if(pessoa[j].posX <= pessoa[msg_ret.client_id].posX + 100){
                  if(pessoa[j].posY <= pessoa[msg_ret.client_id].posY + 100){
                    pessoa[j].vida--;
                  }
                  else if(pessoa[j].posY <= pessoa[msg_ret.client_id].posY - 100){
                    pessoa[j].vida--;
                  }
                }
                else if(pessoa[j].posX <= pessoa[msg_ret.client_id].posX - 100){
                  if(pessoa[j].posY <= pessoa[msg_ret.client_id].posY + 100){
                    pessoa[j].vida--;
                  }
                  else if(pessoa[j].posY <= pessoa[msg_ret.client_id].posY - 100){
